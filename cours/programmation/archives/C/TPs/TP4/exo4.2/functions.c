...
// on va ensuite si on trouve plusieurs fois la meme valeur la suprimer
void removeValue(int array[], int *size, int value) {
    for (int i = 0; i < *size; i++) {
        if (array[i] == value) {
            // Deplace tous les elements suivants d'une position vers la gauche
            for (int j = i; j < *size - 1; j++) {
                array[j] = array[j + 1];
            }
            (*size)--;  // Reduit la taille du tableau
            i--;  // Verifie de nouveau la position actuelle
        }
    }
}